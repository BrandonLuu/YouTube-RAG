...
if __name__ == "__main__":
    ...
    def test_channel_query():
        ...
        query = "what is the total view count for Channel5YouTube?"
        # query = "What is the total subscriber count for Channel5Youtube?"
        # query = "what are the total number of videos for Channel5YouTube?"
        # query = "What are the topics for Channel5Youtube?"

        channel_name = "@Channel5YouTube"

        # search if cached to avoid duplicate entries in the DB
        if not find_channel_name(channel_name):
            get_and_save_channel_analytics(channel_name)
    ...
    def test_video_query():
        ...
        if not find_video_id(video_id):
            get_and_save_video_comments(video_id, max_results=10)
            wait_for_vector_search_ready()
